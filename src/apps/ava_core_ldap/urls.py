...
urlpatterns = patterns('',
    ...
    url(r'^aduser/(?P<pk>\d+)/view/$', login_required(views.ActiveDirectoryUserDetail.as_view()), name='ad-user-detail'),
    url(r'^aduser/(?P<pk>\d+)/update/$', login_required(views.ActiveDirectoryUserUpdate.as_view()), name='ad-user-update'),
    url(r'^aduser/(?P<pk>\d+)/delete/$', login_required(views.ActiveDirectoryUserDelete.as_view()), name='ad-user-delete'),
    
    url(r'^(?P<pk>\d+)/adgroup/$', login_required(views.ActiveDirectoryGroupIndex.as_view()), name='ad-group-index'),
    url(r'^(?P<pk>\d+)/adgroup/new/$',login_required(views.ActiveDirectoryGroupCreate.as_view()),name='ad-group-create'),
    url(r'^adgroup/(?P<pk>\d+)/view/$', login_required(views.ActiveDirectoryGroupDetail.as_view()), name='ad-group-detail'),
    url(r'^adgroup/(?P<pk>\d+)/update/$', login_required(views.ActiveDirectoryGroupUpdate.as_view()), name='ad-group-update'),
