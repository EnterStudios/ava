...
class IdentityValidation(object):
...
def validate_skype(skype_username):
    ...
    """
    if IdentityValidation.VALIDATE_SKYPE.match(skype_username) is None:
...
def validate_twitter(twitter_username):
    ...
    """
    if IdentityValidation.VALIDATE_TWITTER.match(twitter_username) is None:
